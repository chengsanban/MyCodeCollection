...
HRESULT Setup()
{
	...
	if (FAILED(res))
	{
		...
		SAFE_RELEASE(adjBuffer); // free
		return res;
	}

	// Generate the progressive mesh. 
	res |= D3DXGeneratePMesh(
		pMesh,
	...
	SAFE_RELEASE(pMesh);  // done w/ source mesh
	...
	DWORD maxFaces = pPMesh->GetMaxFaces();
	pPMesh->SetNumFaces(maxFaces);
...
HRESULT Present()
{
	...
	if (pDirect3DDevice9)
	{
		...
		int numFaces = pPMesh->GetNumFaces();
		...
		if (::GetAsyncKeyState('A') & 0x8000f)
		{
			...
			pPMesh->SetNumFaces(numFaces + 1);
			if (pPMesh->GetNumFaces() == numFaces)
				pPMesh->SetNumFaces(numFaces + 2);
		}
		...
		for (int i = 0; i < mtrlsVector.size(); i++)
		{
			...
			res |= pDirect3DDevice9->SetMaterial(&YELLOW_MTRL);
			res |= pDirect3DDevice9->SetRenderState(D3DRS_FILLMODE, D3DFILL_WIREFRAME);
			pPMesh->DrawSubset(i);
			res |= pDirect3DDevice9->SetRenderState(D3DRS_FILLMODE, D3DFILL_SOLID);
		}
